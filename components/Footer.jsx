...
const Footer = () => {
    ...
    const TiktokIcon = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#90A1B9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5"></path></svg>)
    const TelegramIcon = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#90A1B9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>)

    const linkSections = [
        {
            title: "ՏԵՍԱԿԱՆԻ",
            links: [
                { text: "🎧 Ականջակալներ", path: '/', icon: null },
                { text: "⌚️ Խելացի ժամացույցներ", path: '/', icon: null },
                { text: "📱 Սմարթֆոններ", path: '/', icon: null },
                { text: "💻 Նոութբուքեր", path: '/', icon: null },
            ]
        },
        {
            title: "ԾԱՌԱՅՈՒԹՅՈՒՆՆԵՐ",
            links: [
                { text: "☘️ 3 ամիս երաշխիք", path: '/', icon: null },
                { text: "🚚 Անվճար առաքում", path: '/', icon: null },
                { text: "🕐 11:00-23:00", path: '/', icon: null },
                { text: "🤖 Telegram Բոտ (Ապրանքներ)", path: 'https://t.me/iclone_evn_bot', icon: null },
            ]
        },
        {
            title: "ԿԱՊ",
            links: [
                { text: "📞 096-683-188", path: 'tel:+37496683188', icon: PhoneIcon },
                { text: "💬 Telegram Reviews", path: 'https://t.me/iclone_evn_reviews', icon: TelegramIcon },
                { text: "📍 Երևան, Հայաստան", path: '/', icon: MapPinIcon }
            ]
        }
    ];

    const socialIcons = [
        { icon: InstagramIcon, link: "https://www.instagram.com/iclone.evn" },
        { icon: TiktokIcon, link: "https://www.tiktok.com/@iclone.evn" },
        { icon: TelegramIcon, link: "https://t.me/iclone_evn_bot" },
    ]
    ...
    return (
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/" className="text-4xl font-semibold text-slate-700">
                            <span className="text-slate-900">IClone</span><span className="text-green-600">EVN</span><span className="text-green-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm leading-6">
                            🎧⌚️ Առաջին <b>Luxe Copy</b> օնլայն խանութը Հայաստանում 🇦🇲<br/>
                            Մենք առաջարկում ենք լավագույն որակի սարքավորումներ՝ 3 ամիս երաշխիքով և անվճար առաքմամբ ՀՀ ողջ տարածքում։
                        </p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} target="_blank" className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:scale-110 hover:bg-slate-200 transition-all rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[60%] gap-5 text-sm ">
                        {linkSections.map((section, index) => (
                            <div key={index} className="min-w-[150px]">
                                <h3 className="font-bold text-slate-700 md:mb-5 mb-3 uppercase tracking-wider">{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="hover:text-green-600 transition" target={link.path.startsWith('http') ? "_blank" : "_self"}>
                                                {link.text}
                                            </Link>
                                        </li>
                                    ))}
                ...
                <div className="flex flex-col md:flex-row justify-between items-center py-6 gap-4 text-sm text-slate-500">
                    <p>Հեղինակային իրավունքներ {new Date().getFullYear()} © <b>IClone EVN</b>. Բոլոր իրավունքները պաշտպանված են:</p>
                    <p className="flex items-center gap-1">
                        Պատրաստված է՝ 
                        <Link href="https://www.instagram.com/mkheyan_engineering/" target="_blank" className="font-bold text-slate-800 hover:text-green-600 transition border-b border-slate-800">
                            MKHEYAN ENGINEERING
                        </Link>
                    </p>
                </div>
